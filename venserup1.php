...
session_start();
		...
		{
			$idd=$_POST['au'];
$sv4="select *from vender where id='".$idd."'";

$svq4=mysqli_query($cnt,$sv4);
if($svq4)
	{
	if(mysqli_fetch_row($svq4)>0)
	{
	$svq4=mysqli_query($cnt,$sv4);
	echo'<form action="venserup.php" method="POST" enctype="multipart/form-data">';
			while($r=mysqli_fetch_row($svq4))
			{
 ...
 {
	 $idd=$_POST['au'];
 $sv4="delete from vender where id='".$idd."'";
